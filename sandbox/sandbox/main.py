...
async def initialize(init_data: InitType):
    ...
    session_path: str = init_data.path or "/default/path"
...
async def run_command(data: dict):
    ...
    if mode == ExecutionMode.STREAM:
        ...
        process_obj: Process = Process(
            process.pid, process, process.stdout, process.stderr
        )
...
async def check_status(sid, data):
    process_id: str = data.get("process_id")
    session: Session = session_manager.get_session_by_sid(sid)
...
async def get_all_file_paths(session_id: str, regexes: List[str] = None):
    if regexes is None:
        regexes = []
    session: Session = session_manager.get_session(session_id)
    if not session:
...
@app.post("/get_website_console_logs")
async def runtime_test(body: dict):
    session_id: str = body.get("session_id")
    path: str = body.get("full_url")

    if not session_id or not path:
        raise HTTPException(
            status_code=400, detail="Session ID, process ID, and path are required"
        )
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            await page.wait_for_timeout(2000)  # Wait for 2s to catch any delayed errors
        except Exception as e:
            await browser.close()
            return {"error": str(e)}
